...
class LikesGetController extends AbstractActionController
{
    public function likesGetAction()
    {
		try {
		    $table = $this->get('LikesTable');
		    $permitted_fields = [
		        '_user_fields' => $this->get('UserTable')->getStandartFields(),
		        '_likes_fields' => $table->getStandartFields(),
		    ];
		    $default_options = [
		    '_user_fields' => ['id'],
		    '_likes_fields' => ['id'],
		    ...
		    $data_list = $table->getLikes($section, $section_id ,$request['filters'], $request['q_options'])->toArray();
		    $data_list = $this->ApiValidateResponse($data_list, $request['q_options']);
		    $request['q_options']['count'] = 1;
		    $total_results = $table->getLikes($section, $section_id, $request['filters'], $request['q_options'])->current()->count;
